...
def choose_target():
    ...
    html = "<h2>Which column would you like to predict?</h2><form method='post' action='/choose'>"
    html += "<select name='target_column'>"
    for col in columns:
        html += f"<option value='{col}'>{col}</option>"
    html += "</select><br><br><input type='submit' value='Next'/></form>"
...
async def ask_values(request: Request):
    ...
    # עמודות קלט (חוץ מהשאלה)
    input_columns = [col for col in my_manager.dfm.columns if col != target_column]

    html = f"<h2>Enter values for classification (Target: {target_column})</h2>"
